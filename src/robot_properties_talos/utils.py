...
def find_paths(robot_name, robot_family="palrobotics"):
    with importlib_resources.path(__package__, "utils.py") as p:
        package_dir = p.parent.absolute()

    resources_dir = path.join(package_dir, "robot_properties_" + robot_name)
    dgm_yaml_path = path.join(
        resources_dir,
        "dynamic_graph_manager",
        "dgm_parameters_" + robot_name + ".yaml",
    )
    urdf_path = path.join(resources_dir + '/urdf/', robot_name + ".urdf")
